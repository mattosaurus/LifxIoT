...
namespace LifxIoT.Api
{
    ...
    public abstract class ApiMethod<TResult> : ApiMethod
    {
        ...
        protected async virtual Task<TResult> Execute(params string[] args)
        {
            ...
            {
                returnValue = await this.OnExecuted(response).ConfigureAwait(continueOnCapturedContext: false);
            }
            else
            {
                await this.OnFailed(response).ConfigureAwait(continueOnCapturedContext: false);
            }
